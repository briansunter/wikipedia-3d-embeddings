...
def fetch_data_from_db():
    ...
    documents = [{"id": row[0], "value": row[1], "embeddings": json.loads(row[2] or [])} for row in cursor.fetchmany(2000)]
...
def export_data_to_json(documents, xyz_coords):
    ...
    for doc, coord in zip(documents, xyz_coords):
        data.append({"id": doc["id"], "value": doc["value"], "x": float(coord[0]), "y": float(coord[1]), "z": float(coord[2])})
    with open("public/data/premapdocuments4.json", "w") as f:
